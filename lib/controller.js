...
Controller.prototype.params = function(name, defaultValue) {
  var paramExists = function(obj) {
    return typeof obj === 'object' && obj.hasOwnProperty(name);
  };

  if (paramExists(this.__req.params)) {
    return this.__req.params[name];
  } else if (paramExists(this.__req.body)) {
    return this.__req.body[name];
  } else if (paramExists(this.__req.query)) {
    return this.__req.query[name];
  } else {
